...
export default createRule<Options, 'forbidden'>({
  name: 'no-array-prototype-entries',
  meta: {
    type: 'problem',
    docs: { description: 'disallow the `Array.prototype.entries` method.', recommended: false },
    schema,
    messages: { forbidden: "ES2015 '{{name}}' method is forbidden." },
  },
  defaultOptions: [{}],
  create(context, options) {
    return definePrototypeMethodHandler(context, options, { Array: ['entries'] });
  },
});
