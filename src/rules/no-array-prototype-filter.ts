...
export default createRule<Options, 'forbidden'>({
  name: 'no-array-prototype-filter',
  meta: {
    type: 'problem',
    docs: { description: 'disallow the `Array.prototype.filter` method.', recommended: false },
    schema,
    messages: { forbidden: "ES5 '{{name}}' method is forbidden." },
  },
  defaultOptions: [{}],
  create(context, options) {
    return definePrototypeMethodHandler(context, options, { Array: ['filter'] });
  },
});
