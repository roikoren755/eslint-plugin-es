...
export default createRule<Options, 'forbidden'>({
  name: 'no-array-prototype-keys',
  meta: {
    type: 'problem',
    docs: { description: 'disallow the `Array.prototype.keys` method.', recommended: false },
    schema,
    messages: { forbidden: "ES2015 '{{name}}' method is forbidden." },
  },
  defaultOptions: [{}],
  create(context, options) {
    return definePrototypeMethodHandler(context, options, { Array: ['keys'] });
  },
});
