...
export default createRule<Options, 'forbidden'>({
  name: 'no-string-prototype-codepointat',
  meta: {
    type: 'problem',
    docs: { description: 'disallow the `String.prototype.codePointAt` method.', recommended: false },
    schema,
    messages: { forbidden: "ES2015 '{{name}}' method is forbidden." },
  },
  defaultOptions: [{}],
  create(context, options) {
    return definePrototypeMethodHandler(context, options, { String: ['codePointAt'] });
  },
});
